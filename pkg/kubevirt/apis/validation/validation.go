...
func ValidateKubevirtSecret(spec *api.KubeVirtProviderSpec, secrets *api.Secrets) []error {
	...
	if _, err := kubevirt.ParseResources(spec.CPUs, spec.Memory); err != nil {
	...
	if spec.DNSPolicy != "" {
		dnsPolicy, err := kubevirt.DNSPolicy(spec.DNSPolicy)
		if err != nil {
